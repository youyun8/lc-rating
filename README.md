...
- **進度管理**：做題記錄存於瀏覽器本地，可選擇透過後端登入進行雲端同步（見 `backend/README.md`）。
...
- 後端：Cloudflare Worker + [Hono](https://hono.dev/)（雲端同步，選用）
