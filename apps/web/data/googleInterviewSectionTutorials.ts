...
const competitiveProgrammingCourseMaterials: Record<number, string> = {
  ...
  5: [
    ...
    "```cpp\nstruct RollingHash {\n    static const long long kMod = 1000000007, kBase = 911382323;\n    vector<long long> h, p;\n    RollingHash(const string& s) : h(s.size() + 1), p(s.size() + 1, 1) {\n        for (int i = 0; i < (int)s.size(); ++i) {\n            h[i + 1] = (h[i] * kBase + s[i]) % kMod;\n            p[i + 1] = p[i] * kBase % kMod;\n        }\n    }\n    long long get(int l, int r) {\n        return (h[r] - h[l] * p[r - l] % kMod + kMod) % kMod;\n    }\n};\n```",
  ...
  9: [
    ...
    "```cpp\nbool hasSkill(int mask, int index) { return mask & (1 << index); }\nint addSkill(int mask, int index) { return mask | (1 << index); }\nint removeSkill(int mask, int index) { return mask & ~(1 << index); }\n```",
    "**Pattern 3：溢位保護**\n\n競程中 `int` 很容易在加總、乘法、距離中溢位。模板：乘法前轉 `long long`，kInf 設成不會在加法後溢位的值。",
    "```cpp\nconst long long kInf = 4e18;\nlong long safeAddCapped(long long a, long long b) {\n    if (a >= kInf || b >= kInf) return kInf;\n    if (a > kInf - b) return kInf;\n    return a + b;\n}\n```",
    "**Pattern 4：Free list**\n\n訊號：需要反覆 allocate/free 固定數量節點。用陣列存節點，用 stack 保存可重用 index，避免碎片。",
    "```cpp\nclass Pool {\n    vector<int> value, free_ids;\npublic:\n    Pool(int n) : value(n) { for (int i = n - 1; i >= 0; --i) free_ids.push_back(i); }\n    int alloc(int x) { if (free_ids.empty()) return -1; int id = free_ids.back(); free_ids.pop_back(); value[id] = x; return id; }\n    void release(int id) { free_ids.push_back(id); }\n};\n```",
  ].join("\n\n"),
  10: [
  ...
  1103: [
    ...
    "```cpp\nvector<int> tin, tout, order;\nvoid dfsOrder(int u, int parent, vector<vector<int>>& g) {\n    tin[u] = order.size();\n    order.push_back(u);\n    for (int v : g[u]) if (v != parent) dfsOrder(v, u, g);\n    tout[u] = order.size();\n}\nbool isAncestor(int u, int v) { return tin[u] <= tin[v] && tin[v] < tout[u]; }\n```",
