...
const studyPlanDeepDiveMaterials: Record<string, string> = {
  ...
  binary_search: [
    ...
    "**例題解析：最小化最大分段和**\n\n給陣列切成最多 k 段，最小化每段和的最大值。若限制 `limit` 越大，越容易切成功，所以 `canSplit(limit)` 單調。檢查方式是從左到右貪心累加，超過 limit 就開新段；這種貪心正確，因為在固定 limit 下，當前段能多放就多放，不會增加段數。",
  ...
  data_structure: [
    ...
    "**例題解析：動態前綴和**\n\n若有多次單點加值與區間和查詢，普通前綴和每次更新要改很多位置。Fenwick tree 用 `index += index & -index` 更新所有負責包含該點的桶；查詢用 `index -= index & -index` 累加前綴桶。",
  ...
  sliding_window: [
    ...
    "```cpp\nlong long atMostKDistinct(vector<int>& nums, int k) {\n    unordered_map<int, int> count;\n    long long answer = 0;\n    int left = 0;\n    for (int right = 0; right < (int)nums.size(); ++right) {\n        if (count[nums[right]]++ == 0) k--;\n        while (k < 0) {\n            if (--count[nums[left++]] == 0) k++;\n        }\n        answer += right - left + 1;\n    }\n    return answer;\n}\n\nlong long exactlyKDistinct(vector<int>& nums, int k) {\n    return atMostKDistinct(nums, k) - atMostKDistinct(nums, k - 1);\n}\n```",
  ].join("\n\n"),
