...
for (auto& e : edges) { g[e[0]].push_back(e[1]); g[e[1]].push_back(e[0]); } // undirected
\`\`\``,
    },
    {
      id: "traversal",
    ...
    for (int s = 0; s < n; s++) if (color[s] == -1) {
        ...
        while (!q.empty()) {
            ...
            for (int v : g[u]) {
                ...
                else if (color[v] == color[u]) return false; // same-color edge
            }
        }
    }
    return true;
...
vector<vector<int>> criticalConnections(int n, vector<vector<int>>& conns) {
    ...
    for (auto& e : conns) { g[e[0]].push_back(e[1]); g[e[1]].push_back(e[0]); }
...
vector<string> findItinerary(vector<vector<string>>& tickets) {
    ...
    while (!st.empty()) {
        ...
        else { auto it = g[u].begin(); string v = *it; g[u].erase(it); st.push(v); }
    }
...
| 207 / 210 Course Schedule I/II | topological sort |
| 127 Word Ladder | BFS over states |
| 743 Network Delay Time | Dijkstra |
| 787 Cheapest Flights K Stops | Bellman-Ford |
| 1631 Path With Minimum Effort | Dijkstra variant |
| 1584 Connect All Points | MST |
| 684 Redundant Connection | DSU cycle |
| 785 Is Graph Bipartite | 2-coloring |
...
| Problem | Technique |
| --- | --- |
| 2421 Number of Good Paths | DSU |
| 2092 Find All People With Secret | DSU by event time |
