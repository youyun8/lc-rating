...
class Solution {
 ...
 private:
  ...
  dijkstra_lazy: {
    summary: "C++17 Template: Dijkstra with Lazy Deletion",
  ...
  zero_one_bfs: {
    summary: "C++17 Template: 0-1 BFS with a Deque",
  ...
  bellman_ford_k_edges: {
    summary: "C++17 Template: Bellman-Ford Limited to K Rounds",
  ...
  merge_sorted_lists: {
    summary: "C++17 Template: Dummy-Head Merge",
    code: "// C++17 Template: Dummy-Head Merge\nclass Solution {\n public:\n  ListNode* mergeTwoLists(ListNode* list1, ListNode* list2) {\n    ListNode dummy(0);\n    ListNode* tail = &dummy;\n\n    while (list1 != nullptr && list2 != nullptr) {\n      if (list1->val <= list2->val) {\n        tail->next = list1;\n        list1 = list1->next;\n      } else {\n        tail->next = list2;\n        list2 = list2->next;\n      }\n      tail = tail->next;\n    }\n    tail->next = list1 != nullptr ? list1 : list2;\n\n    return dummy.next;\n  }\n};",
  },
  lru_cache: {
    summary: "C++17 Template: LRU Cache (List + Hash Map)",
  ...
  insert_delete_getrandom: {
    summary: "C++17 Template: Swap-With-Last Random Set",
  ...
  min_stack: {
    summary: "C++17 Template: Stack with Synchronized Minimum",
...
const TEMPLATE_META: Record<string, TemplateMeta> = {
  ...
  dijkstra_lazy: {
    name: "Dijkstra with Lazy Deletion",
    complexity: "O((n + m) log m)",
    whenToUse:
      "Single-source shortest paths with non-negative edge weights; push improved distances and skip stale heap entries instead of decreasing keys.",
  },
  zero_one_bfs: {
    name: "0-1 BFS with a Deque",
    complexity: "O(n + m)",
    whenToUse:
      "Shortest paths when every edge costs 0 or 1 (often grids where following an arrow is free and changing it pays): push 0-edges to the front and 1-edges to the back.",
  },
  bellman_ford_k_edges: {
    name: "Bellman-Ford Limited to K Rounds",
    complexity: "O(k · m)",
    whenToUse:
      "Cheapest path using at most k+1 edges, or graphs with negative edges where Dijkstra's settled-set argument breaks; snapshot the distances each round.",
  },
  fast_slow_cycle: {
    name: "Floyd Cycle Detection (Tortoise and Hare)",
    complexity: "O(n)",
    whenToUse:
      "Detect a cycle and find its entry in a linked list — or any implicit successor function such as duplicate-finding on array indices — in O(1) space.",
  },
  reverse_linked_list: {
    name: "In-Place Linked-List Reversal",
    complexity: "O(n)",
    whenToUse:
      "Reverse a whole list, a sublist, or fixed-size groups by redirecting one `next` pointer per step while a reversed-prefix/intact-suffix invariant holds.",
  },
  merge_sorted_lists: {
    name: "Dummy-Head Merge",
    complexity: "O(n + m)",
    whenToUse:
      "Splice two sorted lists together; the dummy head turns the first real node into an ordinary case so no branch special-cases an empty result.",
  },
  lru_cache: {
    name: "LRU Cache (List + Hash Map)",
    complexity: "O(1) per operation",
    whenToUse:
      "Maintain recency order with a doubly linked list and O(1) lookup with a hash map of list iterators; splice touched nodes to the front and evict from the tail.",
  },
  insert_delete_getrandom: {
    name: "Swap-With-Last Random Set",
    complexity: "O(1) per operation",
    whenToUse:
      "Support insert, delete, and uniform random sampling by storing values in a vector and filling each deletion hole with the last element.",
  },
  min_stack: {
    name: "Stack with Synchronized Minimum",
    complexity: "O(1) per operation",
    whenToUse:
      "Answer an aggregate (min/max) of the current stack contents by pushing the running aggregate alongside every value and popping them together.",
  },
};
...
> = {
  ...
  dijkstra_lazy: {
    lc: { id: 743, slug: "network-delay-time", title: "Network Delay Time" },
  },
  zero_one_bfs: {
    lc: {
      id: 1368,
  ...
  reverse_linked_list: {
    lc: { id: 206, slug: "reverse-linked-list", title: "Reverse Linked List" },
  },
  merge_sorted_lists: {
    lc: {
      id: 21,
  ...
  lru_cache: {
    lc: { id: 146, slug: "lru-cache", title: "LRU Cache" },
  },
  insert_delete_getrandom: {
    lc: {
      id: 380,
  ...
  min_stack: {
    lc: { id: 155, slug: "min-stack", title: "Min Stack" },
  },
};
...
const ENUMERATION_TAXONOMY: string = [
  ...
  "",
  "| Viewpoint | What you enumerate | Invariant maintained | Representative problem |",
  "|---|---|---|---|",
  '| Enumerate the owner / contributor | each element as the one "responsible" for a subarray | every subarray is counted by exactly one owner | 907. Sum of Subarray Minimums |',
  "| Enumerate the right endpoint | fix the right end, count valid left ends | the window/state is valid for the current right end | 2302. Count Subarrays With Score Less Than K |",
  "| Enumerate the pivot / middle | fix the center element | left-side and right-side counts are independent | 1395. Count Number of Teams |",
  "| Enumerate the contribution unit | each pair/triple's added value | sum over units equals the sum over the whole | 2104. Sum of Subarray Ranges |",
  ...
  "| **Enumerate the cut point** (two-cut / three-segment) | each split position | prefix-left and suffix-right are computed independently | 689. Maximum Sum of 3 Non-Overlapping Subarrays |",
  "| **Enumerate the smaller side** (small-to-large merging) | always iterate the smaller set when merging | total work stays O(n log n) because each element moves O(log n) times | 2003. Smallest Missing Genetic Value in Each Subtree |",
  "| **Enumerate bit by bit** (greedy from the high bit) | each bit from high to low | the prefix of already-fixed bits is optimal | 421. Maximum XOR of Two Numbers in an Array |",
  "| **Enumerate the trigger / event** (event-driven) | each moment the state changes | events are processed in time order and reacted to once | 1834. Single-Threaded CPU |",
  "",
  "#### Practice problems",
  "",
...
const TOPIC_DEFINITIONS: PatternTopicDefinition[] = [
  ...
  {
    slug: "shortest-paths",
    title: "Shortest Paths (Dijkstra / 0-1 BFS / Bellman-Ford)",
    tagline:
      "Choose the right relaxation engine — heap, deque, or round-limited sweep — for minimum-cost paths on weighted graphs.",
    icon: "Waypoints",
    group: "Graph Patterns",
    concept: [
      "Imagine dropping a pebble into a pond where the water spreads faster along some channels than others. The ripple reaches every point exactly when the cheapest route to it has been fully traveled. Shortest-path algorithms simulate that ripple: instead of trying routes one by one, they grow a frontier outward in order of accumulated cost, so the first time the ripple touches a node is provably the cheapest way to get there.",
      "All three engines share one primitive — relaxation: if `dist[u] + w(u, v) < dist[v]`, improve `dist[v]`. They differ only in the order relaxations happen: Dijkstra orders by total cost with a heap, 0-1 BFS with a deque, and Bellman-Ford sweeps every edge in rounds.",
      "Contrast with plain BFS: BFS counts hops, which equals shortest distance only when every edge weighs the same. The moment weights differ, visit order must follow cost, not hop count.",
    ],
    motivation: [
      "Brute force enumerates entire paths: from the source, try every neighbor, recurse, and keep the cheapest total reaching the target. On a graph with cycles the path count is unbounded, and even on a DAG it grows exponentially.",
      "Example: 4 nodes with edges `0→1 (1)`, `0→2 (5)`, `1→2 (1)`, `2→3 (1)`. Path enumeration re-derives the `0→1→2` prefix for every continuation. Dijkstra settles node 2 at cost 2 once and never reconsiders it.",
      "The repeated work is recomputing the best way into a node for every path passing through it. Storing one number per node — the best distance so far — collapses all those prefixes into a single relaxation per edge.",
    ],
    whenUse: [
      "If you see minimum cost / time / effort to reach a target on a weighted graph or grid, think Dijkstra with a min-heap.",
      "If every edge costs 0 or 1 (following an arrow is free, changing it pays), think 0-1 BFS with a deque.",
      "If you see at most k stops/edges, or negative edge weights, think round-limited Bellman-Ford with a snapshot per round.",
      "If the cost of a path is its maximum edge instead of the sum, think minimax Dijkstra: relax with `max(d, w)` instead of `d + w`.",
      "If the answer is a probability product to maximize, think Dijkstra on a max-heap with multiplication.",
    ],
    coreIdea: [
      "Build an adjacency list (or treat grid cells as nodes with 4-neighbor edges).",
      "Initialize `dist[src] = 0` and every other distance to a safe infinity.",
      "Pick the engine by edge weights: all equal → plain BFS; 0/1 → deque; non-negative → heap; negative or edge-limited → Bellman-Ford rounds.",
      "Extract the cheapest frontier node and relax its outgoing edges, pushing improved distances rather than decreasing keys.",
      "Skip stale heap entries (`d > dist[node]`) so each node is expanded once.",
      "Stop early when the target is popped — its distance is final at that moment.",
    ],
    invariant:
      "**Settled-Frontier Invariant.** With non-negative weights, when a node is popped from the heap with distance `d`, no other route to it can be cheaper: any alternative path must leave the settled region through some frontier node whose key is already ≥ `d`, and extending a path never decreases its cost. Bellman-Ford replaces this with a round invariant — after round `i`, `dist[v]` is the cheapest cost using at most `i` edges — which survives negative weights because it never claims a node is finished.",
    variants: [
      "Minimax / bottleneck path: relax with `max(d, w)`; covers minimum-effort and earliest-time-to-swim grids. Example: edges `0→1 (4)`, `1→2 (3)`, `0→2 (6)` — the two-hop route costs `max(4, 3) = 4`, beating the direct edge's `6` even though its sum `7` is larger.",
      "Maximum probability: max-heap with product relaxation; non-negativity becomes factors in `(0, 1]`.",
      "Counting shortest paths: alongside `dist`, maintain `ways[v]` summed whenever an equal-cost route arrives.",
      "Multi-source: push every source at distance 0 — the ripple grows from all of them at once.",
      "K-stops cheapest flight: Bellman-Ford with exactly `k + 1` rounds and a per-round snapshot.",
    ],
    templateKeys: ["dijkstra_lazy", "zero_one_bfs", "bellman_ford_k_edges"],
    complexity: [
      "Dijkstra with lazy deletion: O((n + m) log m) time and O(n + m) space; the heap may hold duplicates, which the staleness check filters out.",
      "0-1 BFS: O(n + m) time — each node enters the deque at most twice; the hidden cost is deciding which edges are truly 0-weight.",
      "Bellman-Ford limited to k rounds: O(k · m) time; the full algorithm uses n − 1 rounds, and one extra round detects negative cycles.",
    ],
    mistakes: [
      "Running Dijkstra with negative edges. Counter-example: edges `0→1 (2)`, `0→2 (5)`, `2→1 (-4)` — node 1 is settled at distance 2, but the true distance is 1; the settled-set proof needs non-negative weights.",
      "Forgetting the staleness check. Without `if (d > dist[node]) continue;` a node is expanded once per heap duplicate, inflating the cost toward O(nm log m) on dense improvement chains.",
      "Relaxing from the current round's values in k-stops Bellman-Ford. Without the snapshot, one round can chain several edges, so the at-most-k constraint silently breaks.",
      "Summing distances in `int`. Counter-example: a path of 10^5 edges of weight 10^9 overflows; accumulate in `long long` and use a divided infinity such as `LLONG_MAX / 4`.",
    ],
    practice: [
      {
        id: 743,
        ...
        rating: 1800,
        difficulty: "Medium",
        subPattern: "plain Dijkstra",
        why: "The reference single-source problem: build the graph, run Dijkstra, read the farthest node.",
        order: 1,
        tier: "Core Practice",
      },
      {
        ...
        rating: 1786,
        difficulty: "Medium",
        subPattern: "k-round Bellman-Ford",
        why: "Edge-limited relaxation with a per-round snapshot — the case where Dijkstra is wrong.",
        order: 2,
        tier: "Core Practice",
      },
      {
        ...
        rating: 1846,
        difficulty: "Medium",
        subPattern: "max-heap product Dijkstra",
        why: "The same engine with multiplication and a max-heap; shows relaxation is generic.",
        order: 3,
        tier: "Core Practice",
      },
      {
        ...
        rating: 1948,
        difficulty: "Medium",
        subPattern: "minimax Dijkstra on a grid",
        why: "Relax with max(d, |height difference|) instead of a sum.",
        order: 4,
        tier: "Core Practice",
      },
      {
        ...
        difficulty: "Hard",
        subPattern: "0-1 BFS",
        why: "Free moves along arrows, paid redirections — the canonical deque problem.",
        order: 5,
        tier: "Advanced Practice",
      },
      {
        ...
        rating: 2095,
        difficulty: "Medium",
        subPattern: "Dijkstra + path counting",
        why: "Carry a ways[] array updated on equal-cost arrivals, with modular arithmetic.",
        order: 6,
        tier: "Advanced Practice",
      },
      {
        ...
        rating: 2097,
        difficulty: "Hard",
        subPattern: "bottleneck path",
        why: "Minimize the maximum cell on the path; compare with binary search + BFS.",
        order: 7,
        tier: "Advanced Practice",
      },
      {
        ...
        rating: 2138,
        difficulty: "Hard",
        subPattern: "0-1 BFS on a grid",
        why: "Obstacles cost 1, empty cells cost 0 — recognize the binary weights.",
        order: 8,
        tier: "Advanced Practice",
      },
      {
        ...
        rating: 2154,
        difficulty: "Medium",
        subPattern: "multi-source + maximin",
        why: "Multi-source BFS computes safety first, then a maximin path search runs on top.",
        order: 9,
        tier: "Challenge Practice",
      },
      {
        ...
        rating: 2381,
        difficulty: "Hard",
        subPattern: "Dijkstra with waiting",
        why: "Relaxation must round waiting time up by edge parity — a state-design twist.",
        order: 10,
        tier: "Challenge Practice",
      },
    ],
    recognition: [
      "Are edge weights equal (BFS), binary (deque), non-negative (heap), or negative / edge-limited (Bellman-Ford rounds)?",
      "Is the path cost a sum, a maximum, or a product — and did I adjust the relaxation accordingly?",
      "Did I skip stale heap entries and use an overflow-safe infinity?",
      "Can I stop as soon as the target pops, instead of settling the whole graph?",
    ],
    related: [
      "graph-traversal",
      "heap-patterns",
...

  {
    ...
    title: "Linked List Patterns (Fast/Slow Pointers and In-Place Reversal)",
    tagline:
      "Tortoise-and-hare pointers, dummy heads, and one-pointer-at-a-time reversal solve cycle, middle, and reorder problems in O(1) space.",
    icon: "Link",
    group: "Data Structure Patterns",
    concept: [
      "Put two runners on the same track and start them together, one running twice as fast. On a straight track the fast runner simply finishes first; on a circular track the fast runner laps the slow one and they meet. Linked-list problems exploit exactly this: a meeting certifies a cycle, and where the runners stand when one finishes encodes positions — like the middle — that you could not know without measuring the list first.",
      "Three primitives cover most problems: fast/slow pointers (middle, cycles, k-th from the end), in-place reversal (redirect one `next` pointer per step), and the dummy head (a sentinel that makes the first real node an ordinary case).",
      "Contrast with arrays: no random access and no backward links, so every technique must be expressible as a forward walk that rewires at most a constant number of pointers per step.",
    ],
    motivation: [
      "Brute force for detecting a cycle stores every visited node in a hash set — O(n) extra memory — or walks n steps and declares a cycle if the list has not ended, which needs the unknown length n in advance.",
      "Example: find the middle by first counting nodes (one full pass), then walking n/2 steps (a second pass). Fast/slow pointers do it in one pass — when fast hits the end, slow stands on the middle.",
      "The repeated work is re-measuring positions the pointers can encode implicitly: speed ratios and head starts replace explicit counting passes.",
    ],
    whenUse: [
      "If you see cycle detection or find where the cycle starts, think tortoise and hare, then reset one pointer to the head.",
      "If you see middle of the list or k-th node from the end, think two pointers with a speed ratio or a k-node head start.",
      "If you see reverse a list / sublist / nodes in groups, think in-place reversal with a `previous`/`current` pair.",
      "If you see palindrome or reorder, think: find the middle, reverse the second half, then walk the halves together.",
      "If the head itself may be removed or replaced, think dummy head.",
      "If you see duplicate number in an array of values 1..n, think of `i → nums[i]` as an implicit linked list and run cycle detection.",
    ],
    coreIdea: [
      "Anchor a dummy node before the head whenever the first node can change.",
      "Advance `slow` by one and `fast` by two; stop when `fast` or `fast->next` is null (no cycle) or the pointers meet (cycle).",
      "For the cycle entry, reset one pointer to the head and advance both by one; they meet at the entrance.",
      "For reversal, keep `previous` and `current`; each step saves `current->next`, redirects it to `previous`, and shifts both right.",
      "For reorder/palindrome, compose the primitives: middle → reverse the second half → merge or compare.",
      "Restore the list afterward if the problem requires non-destructive inspection.",
    ],
    invariant:
      "**Distance-Gap Invariant.** Walking `slow` one step and `fast` two grows the gap between them by exactly one per step, and inside a cycle of length `c` the gap counts modulo `c` — so the gap must hit 0 within `c` steps once both pointers are inside, forcing a meeting. For the entry point: if the head is `a` nodes from the entrance and the meeting happens `b` nodes into the cycle, then `a ≡ c − b (mod c)`, so two pointers advanced one step at a time from the head and from the meeting point arrive at the entrance together. Reversal maintains its own invariant: the prefix before `current` is fully reversed and the suffix from `current` is untouched, so stopping at any moment leaves a consistent list.",
    variants: [
      "Cycle entry (Linked List Cycle II): meet, reset one pointer to the head, advance in lockstep.",
      "K-th from the end: give the lead pointer a k-node head start and advance both until it falls off.",
      "Reverse in k-groups: reverse k nodes at a time, reconnecting each group's old head as the new tail.",
      "Palindrome check: middle + reverse second half + parallel walk (optionally restore the list).",
      "Array as implicit list: Find the Duplicate Number runs Floyd on `i → nums[i]`.",
    ],
    templateKeys: [
      "fast_slow_cycle",
      "reverse_linked_list",
    ...
    complexity: [
      "Every primitive is O(n) time with O(1) extra space — that constant space is usually the entire point versus hash-set or array-copy baselines.",
      "Floyd's meeting takes at most n + c steps; the entry-finding phase adds at most n more.",
      "Recursive list solutions cost O(n) stack space, silently forfeiting the O(1)-space advantage.",
    ],
    mistakes: [
      "Dereferencing `fast->next->next` without checking `fast->next`. Counter-example: a two-node acyclic list crashes on the second advance unless both `fast` and `fast->next` are tested.",
      "Losing the suffix during reversal. Forgetting to save `current->next` before redirecting it orphans the rest of the list; the save must come first.",
      "Off-by-one on the middle for even lengths. The `while (fast && fast->next)` loop leaves `slow` on the second middle node; palindrome splitting must pick one convention and stick to it.",
      "Skipping the dummy head when removing. Counter-example: removing the 2nd node from the end of a 2-node list deletes the head itself — only a sentinel makes that an ordinary unlink.",
    ],
    practice: [
      {
        id: 206,
        ...
        why: "The reversal primitive in isolation; master the previous/current dance.",
        order: 1,
        tier: "Core Practice",
      },
      {
        ...
        rating: 1250,
        difficulty: "Easy",
        subPattern: "fast/slow middle",
        why: "Pure speed-ratio positioning: one pass, no counting.",
        order: 2,
        tier: "Core Practice",
      },
      {
        ...
        difficulty: "Easy",
        subPattern: "cycle detection",
        why: "Floyd's meeting argument with no extra memory.",
        order: 3,
        tier: "Core Practice",
      },
      {
        ...
        why: "The sentinel removes every first-node special case.",
        order: 4,
        tier: "Core Practice",
      },
      {
        ...
        rating: 1400,
        difficulty: "Medium",
        subPattern: "head-start gap",
        why: "A k-node head start replaces a length-counting pass; the dummy head handles head removal.",
        order: 5,
        tier: "Core Practice",
      },
      {
        ...
        rating: 1550,
        difficulty: "Medium",
        subPattern: "cycle entry",
        why: "The reset-to-head phase — the modular-arithmetic heart of the chapter.",
        order: 6,
        tier: "Advanced Practice",
      },
      {
        ...
        rating: 1450,
        difficulty: "Easy",
        subPattern: "middle + reverse + compare",
        why: "Composes three primitives into an O(1)-space check.",
        order: 7,
        tier: "Advanced Practice",
      },
      {
        ...
        rating: 1700,
        difficulty: "Medium",
        subPattern: "middle + reverse + interleave",
        why: "The same composition, but splicing instead of comparing.",
        order: 8,
        tier: "Advanced Practice",
      },
      {
        ...
        why: "Reversal with boundary reconnection on both sides.",
        order: 9,
        tier: "Advanced Practice",
      },
      {
        ...
        rating: 1750,
        difficulty: "Medium",
        subPattern: "implicit list cycle",
        why: "Floyd on i → nums[i]: the cycle entry equals the duplicate value.",
        order: 10,
        tier: "Advanced Practice",
      },
      {
        ...
        rating: 1950,
        difficulty: "Hard",
        subPattern: "grouped reversal",
        why: "Repeated sublist reversal with strict group accounting.",
        order: 11,
        tier: "Challenge Practice",
      },
      {
        ...
        rating: 1850,
        difficulty: "Hard",
        subPattern: "k-way dummy-head merge",
        why: "Scale the merge with a heap or divide and conquer.",
        order: 12,
        tier: "Challenge Practice",
      },
    ],
    recognition: [
      "Can a speed ratio or a head start replace an explicit length-counting pass?",
      "Does my loop guard check both `fast` and `fast->next` before the double advance?",
      "Am I saving `current->next` before redirecting it during reversal?",
      "Could the head change — and if so, did I anchor a dummy node?",
    ],
    related: [
      "two-pointers-opposite",
      "heap-patterns",
...

  {
    ...
    title: "Design Problems (LRU Cache and O(1) Structures)",
    tagline:
      "Compose hash maps with lists, vectors, and heaps so every promised operation — get, put, evict, sample — meets its stated time bound.",
    icon: "Blocks",
    group: "Data Structure Patterns",
    concept: [
      "A busy kitchen keeps its most-used pans on the front hooks: grab one, cook, hang it back at the front, and the pan on the last hook is always the one nobody has touched the longest. When a new pan arrives and the rack is full, the back pan goes to storage. The LRU cache is exactly this rack — a recency-ordered list — plus a labeled index that tells you instantly which hook each pan hangs on, so you never search the rack.",
      "Design problems hand you an operation contract (each call with its required complexity) instead of an input/output puzzle. The craft is composing two or three primitive structures so each covers the operation the others cannot do fast.",
      "Contrast with algorithm problems: there is no final answer to compute — correctness means every operation preserves the structures' mutual consistency while hitting its bound.",
    ],
    motivation: [
      "Brute force for an LRU cache stores pairs in a vector: get scans in O(n), and refreshing recency means erasing and re-appending — also O(n).",
      "Example: capacity 2 with calls put(1), put(2), get(1), put(3). The vector scan re-derives each key's position on every call, and the eviction scan re-derives the oldest entry every time the cache is full.",
      "The repeated work is searching for what an index could remember: a hash map remembers each key's location (killing the get scan), and a doubly linked list remembers recency order (killing the eviction scan and making move-to-front O(1)).",
    ],
    whenUse: [
      "If you see design X supporting operations in O(1) / O(log n), think: list one structure per required operation, then connect them.",
      "If you see recency or frequency eviction (LRU, LFU), think doubly linked list(s) plus a hash map of iterators.",
      "If you see random sampling with insert/delete (getRandom), think vector + swap-with-last + index map.",
      "If you see a running aggregate of a stream (median, minimum), think two heaps or a synchronized auxiliary stack.",
      "If you see implement one container with others (queue via stacks), think amortized transfer between an in-box and an out-box.",
    ],
    coreIdea: [
      "Write down the operation contract: every call and its required bound.",
      "Pick one primitive per hard requirement: hash map for lookup, linked list for ordered O(1) splice, vector for random access, heap for extremes.",
      "Connect them: the map stores positions (iterators or indices) inside the order structure, never copies of the data.",
      "Keep one source of truth per fact, and update every mirror inside the same call — no deferred bookkeeping.",
      "Walk each operation and verify it touches only O(1) (or O(log n)) work, including the eviction path.",
      "Test the contract edges: capacity 1, re-inserting an existing key, deleting the last element.",
    ],
    invariant:
      "**Mirror-Consistency Invariant.** After every operation, each auxiliary structure is an exact projection of the primary state: the hash map contains precisely the live keys mapped to their true positions, and the order structure contains precisely the live entries in their claimed order. Every operation that moves data updates all mirrors before returning, so any single structure can answer its queries without consulting the others. Eviction correctness follows directly: the tail of the recency list is genuinely least-recent because every access splices the touched node to the front.",
    variants: [
      "LRU cache: doubly linked list + hash map of iterators; evict at the tail.",
      "LFU cache: per-frequency lists + key map + a minimum-frequency cursor.",
      "Swap-with-last random set: vector + index map; deletion fills the hole with the tail element.",
      "Two-heap median: max-heap of the lower half, min-heap of the upper half, sizes balanced within one.",
      "Aggregate stack/queue: auxiliary stack of running minimums, or a queue built from two stacks with amortized O(1) transfer.",
    ],
    templateKeys: ["lru_cache", "insert_delete_getrandom", "min_stack"],
    complexity: [
      "LRU operations are O(1) worst case — hash lookup plus list splice; space is O(capacity), and the iterator map is the hidden cost.",
      "Swap-with-last gives O(1) insert/delete/sample; the hidden constraint is that element order is destroyed, which is exactly what uniform sampling permits.",
      "Two-heap median is O(log n) insert and O(1) query; supporting removals adds a lazy-deletion cleanup amortization.",
    ],
    mistakes: [
      "Storing values in the map and order in the list without linking them. Counter-example: get(key) finds the value in O(1) but must scan the list to refresh recency — the map must store list iterators, not values alone.",
      "Invalidating iterators by erase-and-reinsert. `std::list::splice` preserves iterators while moving a node to the front; erase plus insert does not.",
      "Treating put on an existing key as an insert. Counter-example: capacity 2 with put(1), put(2), put(1, new), put(3) must evict 2, not 1 — the existing-key path refreshes recency and updates the value without eviction.",
      "In swap-with-last, erasing the target's index entry before updating the moved element's. When deleting the last element the two entries collide; update the moved index first or guard the equal case.",
    ],
    practice: [
      {
        id: 155,
        ...
        rating: 1350,
        difficulty: "Medium",
        subPattern: "synchronized aggregate stack",
        why: "The smallest composition: one auxiliary array mirrors a running minimum.",
        order: 1,
        tier: "Core Practice",
      },
      {
        ...
        rating: 1250,
        difficulty: "Easy",
        subPattern: "amortized two-stack transfer",
        why: "The in-box/out-box trick and its amortized O(1) argument.",
        order: 2,
        tier: "Core Practice",
      },
      {
        ...
        rating: 1300,
        difficulty: "Easy",
        subPattern: "buckets from scratch",
        why: "Build the primitive everything else leans on.",
        order: 3,
        tier: "Core Practice",
      },
      {
        ...
        why: "Vector + index map; the delete is the whole interview.",
        order: 4,
        tier: "Core Practice",
      },
      {
        ...
        rating: 1800,
        difficulty: "Medium",
        subPattern: "list + iterator map",
        why: "The canonical design question: a recency list spliced in O(1).",
        order: 5,
        tier: "Core Practice",
      },
      {
        ...
        rating: 1500,
        difficulty: "Medium",
        subPattern: "ring buffer arithmetic",
        why: "Head/tail modular arithmetic with explicit full/empty states.",
        order: 6,
        tier: "Advanced Practice",
      },
      {
        ...
        rating: 1464,
        difficulty: "Medium",
        subPattern: "paired hash maps",
        why: "Two maps keyed differently must mirror one journey fact consistently.",
        order: 7,
        tier: "Advanced Practice",
      },
      {
        ...
        rating: 1540,
        difficulty: "Medium",
        subPattern: "map + ordered set per value",
        why: "Index updates must erase the stale reverse mapping.",
        order: 8,
        tier: "Advanced Practice",
      },
      {
        ...
        difficulty: "Hard",
        subPattern: "two balanced heaps",
        why: "The classic stream aggregate with a size-balance invariant.",
        order: 9,
        tier: "Advanced Practice",
      },
      {
        ...
        rating: 2100,
        difficulty: "Hard",
        subPattern: "frequency buckets",
        why: "An LRU list inside every frequency bucket plus a min-frequency cursor.",
        order: 10,
        tier: "Challenge Practice",
      },
      {
        ...
        rating: 2050,
        difficulty: "Hard",
        subPattern: "bucket list of counts",
        why: "Doubly linked count buckets keep min and max keys at the ends in O(1).",
        order: 11,
        tier: "Challenge Practice",
      },
    ],
    recognition: [
      "Did I write the per-operation complexity contract down before choosing structures?",
      "Does every fact have one source of truth, with all mirrors updated in the same call?",
      "Is the eviction/deletion path O(1) too, not just the happy path?",
      "Did I test capacity 1, re-inserting an existing key, and deleting the tail element?",
    ],
    related: [
      "hash-map-frequency",
      "heap-patterns",
