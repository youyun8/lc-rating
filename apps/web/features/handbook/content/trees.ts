...
TreeNode* lca(TreeNode* r, TreeNode* p, TreeNode* q) {
    ...
    if (L && R) return r;             // p and q split here -> r is the LCA
...
struct LCA {
    ...
    int lca(int a, int b) {
        ...
        for (int k = 0; k < LOG; k++) if (d & (1 << k)) a = up[k][a]; // lift a up to b
        if (a == b) return a;
...
vector<int> sumOfDistancesInTree(int n, vector<vector<int>>& edges) {
    ...
    function<void(int,int)> post = [&](int u, int p) {
        for (int v : g[u]) if (v != p) { post(v, u); cnt[u] += cnt[v]; ans[u] += ans[v] + cnt[v]; }
    };
    function<void(int,int)> reroot = [&](int u, int p) {
        for (int v : g[u]) if (v != p) { ans[v] = ans[u] - cnt[v] + (n - cnt[v]); reroot(v, u); }
    };
    post(0, -1); reroot(0, -1);
    return ans;
}
...
vector<int> morrisInorder(TreeNode* root) {
    ...
    while (cur) {
        if (!cur->left) { out.push_back(cur->val); cur = cur->right; }
            ...
            if (!pre->right) { pre->right = cur; cur = cur->left; }    // create thread
            else { pre->right = nullptr; out.push_back(cur->val); cur = cur->right; } // remove
        }
    }
    return out;
}
...
| --- | --- |
| 94 / 144 / 145 Traversals | recursion / stack |
| 102 / 103 / 199 Level / Zigzag / Right View | BFS |
| 98 Validate BST | bounds recursion |
| 230 Kth Smallest in BST | inorder |
| 235 / 236 LCA (BST / binary) | descent / postorder |
| 543 Diameter | tree DP |
| 124 Max Path Sum | tree DP with drop |
| 337 House Robber III | tree DP with state |
| 105 / 106 Construct from Traversals | divide & conquer |
...
| Problem | Technique |
| --- | --- |
| 2458 Height of Binary Tree After Subtree Removal Queries | DFS in/out order |
| 2467 Most Profitable Path in a Tree | tree + BFS paths |
| 2049 Count Nodes With the Highest Score | tree DP |
