...
export const Q3_SUBTOPICS: Record<number, Q3Subtopic[]> = {
  ...
  93012: [
    sub(
      ...
      {
        what: "動態規劃不是先背公式，而是把暴力遞迴中的重複子問題命名成狀態。每個狀態都要能用一句話說清楚，例如 `dp[i]` 表示處理前 i 個元素後的最佳值，或 `dp[i][k]` 表示處理到 i 且用了 k 份資源的答案。",
        intuition:
          "如果未來只需要過去的摘要，而不需要完整路徑，就能把遞迴樹壓成表。寫法通常從記憶化搜尋開始最安全：先讓遞迴正確，再把參數範圍、依賴方向與初值翻成遞推。",
        when: "- 題目有選或不選、分段、路徑、匹配、有限狀態、最多 K 次等重複選擇。\n- 暴力搜尋會反覆求同一個前綴、區間、mask 或樹上子問題。\n- 答案是最大值、最小值、方案數或可行性。",
        how: "DP 四問：\n\n1. 狀態代表什麼？\n2. 最後一步從哪些舊狀態來？\n3. 哪些狀態是初值，哪些是不可能狀態？\n4. 遍歷順序是否保證依賴已經算好？\n\n```cpp\n// 先寫清楚語意：dp[i] = 處理前 i 個元素的答案\nvector<long long> dp(n + 1, -INF);\ndp[0] = 0;\nfor (int i = 1; i <= n; ++i) {\n  dp[i] = transition(dp, i);\n}\n```",
        mistakes:
          "- `dp[i]` 語意含糊，導致轉移混用前 i 個與以 i 結尾。\n- 不可能狀態初始化成 0，讓非法方案參與 max/min。\n- 遞推順序錯，讀到尚未計算的狀態。\n- 為了省空間太早滾動，反而蓋掉還需要的舊值。",
        checklist: [
          "我能用一句話定義每一維狀態嗎？",
          "轉移描述的是最後一步、最後一段，還是最後一次操作？",
          "初值與不可能狀態分清楚了嗎？",
          "遍歷順序與依賴方向一致嗎？",
        ],
      },
    ...
    sub(
      "入門線性 DP",
      "爬樓梯、打家劫舍、最大子陣列，練的是前一格或前幾格如何推出目前。",
      {
        what: "入門線性 DP 沿著陣列或字串從左到右推。常見狀態有 `dp[i]` 表示到第 i 格的方案數/最小成本，或 `bestEndHere` 表示必須以 i 結尾的最佳值。代表題包含 70、198、53、2140、3186。",
        intuition:
          "線性 DP 的關鍵是分清楚「前綴最佳」與「以目前結尾」。打家劫舍是選/不選目前；最大子陣列是延續上一段或重新開段；跳躍型題目則是做了目前選擇後跳到下一個可處理位置。",
        when: "- 狀態只依賴前一格、前兩格或少數固定位置。\n- 題目沿著一維序列做決策。\n- 出現不能相鄰、以目前結尾、解完要跳過幾格等限制。",
        how: "```cpp\n// 打家劫舍：skip/take 代表處理完前綴後不取/取目前的最佳值\nlong long skip = 0, take = 0;\nfor (int x : nums) {\n  long long nextTake = skip + x;\n  long long nextSkip = max(skip, take);\n  take = nextTake;\n  skip = nextSkip;\n}\nreturn max(skip, take);\n```\n\n最大子陣列則改問：以 i 結尾時，要接前一段還是從 nums[i] 重新開始。",
        mistakes:
          "- 把前綴最佳當成以 i 結尾，導致答案重複接段。\n- 相鄰限制卻從 `dp[i-1]` 選目前。\n- 環形版本未拆成不含首或不含尾。\n- 所有數可能為負時，把答案初始為 0。",
        checklist: [
          "狀態是前綴最佳還是以 i 結尾？",
          "取目前元素後，哪些前狀態被禁止？",
          "答案要看最後一格，還是過程中的最大值？",
    ...
    sub(
      "網格 DP",
      "把二維格子視為只能從已計算方向走來的 DAG，重點是邊界與障礙。",
      {
        what: "網格 DP 用 `dp[r][c]` 表示走到某格的方案數、最小成本或最大收益。若只能從上方、左方或固定方向來，整張圖是有向無環圖，可以按列或按行遞推。",
        intuition:
          "每個格子的答案只依賴鄰近的前驅格。初始化第一列、第一欄與障礙後，就能把路徑問題變成局部轉移。若移動方向更多，通常要先確認是否仍無環；有環時可能改成 BFS/Dijkstra。",
        when: "- 62、64、120、931、3363、3459。\n- 題目是路徑數、最小路徑和、三角形、下降路徑或棋盤收集。\n- 移動方向有限且能排出拓撲順序。",
        ...
        mistakes:
          "- 第一列/第一欄初始化遇到障礙後仍繼續填值。\n- 把可往四方向走的有環問題硬當普通 DP。\n- `dp[r][c]` 是到達成本，卻把離開成本重複加兩次。\n- 壓縮成一維時，左方與上方的讀寫順序搞反。",
        checklist: [
          "每格有哪些合法前驅方向？",
          "起點、障礙、第一列與第一欄初始化正確嗎？",
          "移動關係是否無環，能用遞推處理嗎？",
          "若壓成一維，`dp[c]` 代表上方還是目前列？",
        ],
      },
    ),
    sub(
      "背包 DP",
      "把容量或目標和當成狀態軸，靠遍歷方向區分每件只能拿一次或可重複拿。",
      {
        what: "背包 DP 適合成本/容量/總和有限的選取問題。`dp[c]` 可表示容量 c 下的最大價值、最少物品數、方案數或是否可達。0-1 背包每件只能選一次，完全背包可重複選。",
        intuition:
          "一維背包的核心不變式在遍歷方向。0-1 背包容量倒序，避免同一輪重複使用同一物品；完全背包容量正序，允許本輪已更新的狀態再次接同一物品。方案數題還要分清組合與排列。",
        when: "- 416、494、474、322、518、279。\n- 題目問湊出某個和、容量內最大值、最少硬幣、方案數。\n- 數值總和或容量上限可接受 O(nC)。",
        how: "```cpp\n// 0-1 背包：每個物品最多一次，容量倒序\nfor (auto [w, v] : items) {\n  for (int c = C; c >= w; --c) {\n    dp[c] = max(dp[c], dp[c - w] + v);\n  }\n}\n\n// 完全背包：物品可重複，容量正序\nfor (int coin : coins) {\n  for (int c = coin; c <= C; ++c) {\n    dp[c] = min(dp[c], dp[c - coin] + 1);\n  }\n}\n```",
        mistakes:
          "- 0-1 背包容量正序，讓同一物品被拿多次。\n- 完全背包倒序，阻止了重複使用。\n- 計方案數時，把組合題寫成排列題或反過來。\n- 可達性 DP 沒有先設 `dp[0] = true`。",
        checklist: [
          "每個物品能選幾次？",
          "容量/目標和上限是否能開 DP 陣列？",
          "求最大值、最小值、方案數還是可行性？",
    ...
    sub(
      "序列 DP",
      "LIS、LCS、編輯距離這類題，把前綴或結尾關係寫成二維或可優化的一維狀態。",
      {
        what: "序列 DP 處理子序列、兩字串匹配、編輯操作與遞增關係。LCS/編輯距離常用 `dp[i][j]` 表示兩個前綴的答案；LIS 則可用 `dp[i]` 表示以 i 結尾的最長長度，或用 patience 陣列優化。",
        intuition:
          "兩序列題的最後一步通常是比較最後一個字元：相同就接上 `dp[i-1][j-1]`，不同就從刪/插/替換或跳過一邊轉移。LIS 的本質是找能接在目前之前的最佳結尾，若只要長度，可用二分維護每個長度的最小尾值。",
        when: "- 72、1143、115、300、354、1964。\n- 題目問最長共同/遞增子序列、編輯距離、刪除距離、匹配方案數。\n- 子序列不要求連續，但相對順序不可改。",
        how: "```cpp\n// LCS: dp[i][j] = text1 前 i 個與 text2 前 j 個的 LCS 長度\nfor (int i = 1; i <= n; ++i) {\n  for (int j = 1; j <= m; ++j) {\n    if (a[i - 1] == b[j - 1]) dp[i][j] = dp[i - 1][j - 1] + 1;\n    else dp[i][j] = max(dp[i - 1][j], dp[i][j - 1]);\n  }\n}\n```\n\nLIS 若用二分，`tails[len]` 不是實際答案序列，而是長度 len 的最小可能結尾。",
        mistakes:
          "- 子序列與子陣列混淆，錯把不連續題寫成 Kadane。\n- LCS 二維表少開一列一欄，邊界處理複雜化。\n- LIS 二分時嚴格遞增用 lower_bound，非嚴格用 upper_bound。\n- 計數型序列 DP 更新方向錯，重複使用同一字元。",
        checklist: [
          "題目要求連續還是不連續？",
          "兩個前綴的 i、j 是否代表長度而非下標？",
          "相等與不相等時的轉移都寫完整了嗎？",
          "LIS 是嚴格遞增還是非遞減？",
        ],
      },
    ...
    sub(
      "劃分型 DP",
      "把前綴切成若干段，枚舉最後一段的起點或用資料結構加速。",
      {
        what: "劃分型 DP 用 `dp[i]` 表示前 i 個元素已經切分完成的最佳答案或可行性。轉移會枚舉最後一段 `[j,i)`，檢查這段是否合法，再從 `dp[j]` 接過來。若有 K 段限制，就加上段數維度。",
        intuition:
          "這類題的最後一步不是選單一元素，而是決定最後一段從哪裡開始。暴力枚舉切點是基本版；若檢查區間代價可用前綴和、字典樹、二分或單調結構加速，就能把 O(n²) 壓低。",
        when: "- 139、132、410、3077、3441。\n- 題目問能否分割、最少切幾刀、切成 k 段後最大/最小成本。\n- 限制與一段內的和、最大值、字串是否合法有關。",
        how: "```cpp\n// dp[i] = 前 i 個元素可否被合法切分\nvector<char> dp(n + 1);\ndp[0] = true;\nfor (int i = 1; i <= n; ++i) {\n  for (int j = 0; j < i; ++j) {\n    if (dp[j] && valid(j, i)) {\n      dp[i] = true;\n      break;\n    }\n  }\n}\n```",
        mistakes:
          "- 忘記空前綴 `dp[0]`，導致第一段無法開始。\n- 最後一段的範圍 `[j,i)` 與下標閉區間混用。\n- K 段題把「至多 K」與「恰好 K」混在一起。\n- 區間代價每次重算，實際複雜度多一層。",
        checklist: [
          "最後一段是哪個範圍？",
          "空前綴能作為起點嗎？",
          "段數是至多、至少還是恰好？",
    ...
    sub(
      "狀態機 DP",
      "把每一步的人或物抽象成有限狀態，沿著合法轉移更新。",
      {
        what: "狀態機 DP 適合每天/每個位置都有有限模式的題目，例如股票的持有/未持有、含冷凍期、交替子序列、用過一次刪除權利。`dp[state]` 表示處於某狀態時的最佳值或方案數。",
        intuition:
          "先畫狀態圖，再照箭頭寫轉移。題目增加限制時，不是硬改公式，而是增加狀態或禁止某些轉移。例如股票題多一次交易限制就加交易次數，多冷凍期就加 frozen 狀態。",
        when: "- 121、122、123、188、309、714、376、1911、1186。\n- 每一步可停留在幾種模式，且模式之間有明確轉換。\n- 題目有持有/未持有、買/賣、上升/下降、權利用過/未用過。",
        how: "```cpp\n// 股票無限次交易：cash 未持有，hold 持有\nlong long cash = 0, hold = -INF;\nfor (int price : prices) {\n  long long nextCash = max(cash, hold + price);\n  long long nextHold = max(hold, cash - price);\n  cash = nextCash;\n  hold = nextHold;\n}\n```\n\n若同一天的狀態會互相依賴，先存 next 變數，避免更新順序污染舊狀態。",
        mistakes:
          "- 未持有與冷凍期混成同一狀態，允許非法買入。\n- 不可能的持有狀態初始化成 0。\n- 有交易次數限制時，買入與賣出消耗次數定義不一致。\n- 原地更新造成同一天買賣被重複計算。",
        checklist: [
          "有哪些狀態？每個狀態的語意互斥嗎？",
          "每條轉移代表哪個操作？",
          "不可能狀態是否設成 -INF？",
    ...
    sub(
      "不重疊區間與子陣列 DP",
      "選任務或選多段時，狀態要記住上一個相容位置與已選段數。",
      {
        what: "不重疊問題要求選出的區間、任務或子陣列不能互相覆蓋。區間型常按右端點排序，用二分找最後一個相容區間；子陣列型常用 `dp[k][i]` 或 inside/outside 狀態表示已完成幾段。",
        intuition:
          "核心是最後選的那段與之前答案必須相容。對任務區間，排序後相容前狀態是某個前綴；對連續子陣列，則要分清楚目前是否正在一段內，避免兩段黏在一起或重疊。",
        when: "- 1235、1751、2830、689、3077。\n- 題目要求選不重疊會議、offer、工作或 k 個子陣列。\n- 每段有固定收益或由子陣列和計算收益。",
        how: "```cpp\n// 加權區間排程：dp[i] = 考慮前 i 個已排序區間的最大收益\nsort(intervals, byEnd);\nfor (int i = 1; i <= n; ++i) {\n  int p = lastNonOverlap(i);\n  dp[i] = max(dp[i - 1], dp[p] + profit[i]);\n}\n```\n\nK 段子陣列常維護兩種狀態：已完成 j 段、正在第 j 段。",
        mistakes:
          "- 未排序就二分相容位置。\n- 相容條件 `<` 與 `<=` 沒看清端點定義。\n- 固定 k 段題允許空段或少選段。\n- 正在段內與段外狀態混用，導致重疊。",
        checklist: [
          "區間按哪個端點排序？",
    ...
    sub(
      "區間 DP",
      "狀態是一段 `[l,r]`，通常按長度由短到長合併或枚舉最後分割點。",
      {
        what: "區間 DP 用 `dp[l][r]` 表示子陣列或子字串 `[l,r]` 的答案。轉移可能來自左右端縮小，也可能枚舉中間切點 k，把區間分成兩段合併。",
        intuition:
          "區間 DP 的遍歷順序幾乎都按長度遞增，因為長區間依賴短區間。若最後一步是刪左右端，看 `l+1,r` 或 `l,r-1`；若最後一步是合併兩段，就枚舉切點。",
        when: "- 516、312、1039、1547。\n- 題目在一段連續範圍內做合併、戳氣球、迴文、刪除或括號化。\n- 轉移需要知道子區間的最佳值。",
        how: "```cpp\nfor (int len = 1; len <= n; ++len) {\n  for (int l = 0; l + len - 1 < n; ++l) {\n    int r = l + len - 1;\n    for (int k = l; k < r; ++k) {\n      dp[l][r] = max(dp[l][r], dp[l][k] + dp[k + 1][r] + cost(l, k, r));\n    }\n  }\n}\n```",
        mistakes:
          "- 長度遍歷方向錯，長區間讀到未算好的短區間。\n- 開閉區間混用，`r` 是包含還是不包含不一致。\n- 忘記長度 1 或空區間初值。\n- 合併成本放錯位置，重複計算邊界元素。",
        checklist: [
          "狀態區間是閉區間還是半開區間？",
          "長區間依賴哪些短區間？",
          "基礎長度的答案是什麼？",
    ...
    sub(
      "狀壓 DP",
      "用 mask 表示集合，適合元素數量小但選取順序或子集關係複雜的題。",
      {
        what: "狀壓 DP 把集合壓成整數 mask，第 b 位表示第 b 個元素是否已選。常見狀態有 `dp[mask]`、`dp[mask][last]`，用於排列、TSP、分組、子集可行性與小規模搜尋。",
        intuition:
          "當 n 不大但選取組合很多時，mask 能把「哪些元素已用」變成可索引狀態。轉移通常是往 mask 裡加入一個未選元素，或枚舉 mask 的子集。複雜度多半是 O(2^n n) 或 O(3^n)，要先看 n。",
        when: "- 473、698、847、943、3444。\n- 元素數量約 15 到 20，題目關心集合、排列順序或每個元素用一次。\n- 一般貪心無法決定下一個選誰，但狀態只需記住已選集合。",
        how: "```cpp\nvector<int> dp(1 << n, INF);\ndp[0] = 0;\nfor (int mask = 0; mask < (1 << n); ++mask) {\n  for (int i = 0; i < n; ++i) {\n    if ((mask >> i) & 1) continue;\n    int next = mask | (1 << i);\n    dp[next] = min(dp[next], dp[mask] + cost(mask, i));\n  }\n}\n```",
        mistakes:
          "- n 太大仍開 `2^n` 狀態。\n- `1 << n` 使用 int 溢位，需 `1LL << n`。\n- mask 位元與原陣列下標對不上。\n- 枚舉子集時漏掉空集或全集邊界。",
        checklist: [
          "n 是否小到能承受 `2^n`？",
          "mask 的每一位對應哪個元素？",
          "轉移是加一個元素、移除一個元素，還是枚舉子集？",
          "需要 last 維度記住最後選了誰嗎？",
        ],
      },
    ),
    sub(
      "樹形 DP",
      "以子樹為狀態，自底向上合併孩子，必要時再做換根。",
      {
        what: "樹形 DP 用 `dp[u]` 或 `dp[u][state]` 表示以 u 為根的子樹答案。常見狀態包含選/不選 u、子樹大小、距離、是否被覆蓋。若答案要以每個節點為根，則使用換根 DP。",
        intuition:
          "樹沒有環，天然適合後序遞推。父節點的答案由孩子子樹合併而來；如果父子互相限制，就把 u 的狀態拆開。換根則先算從下往上的貢獻，再把父側貢獻傳給孩子。",
        when: "- 337、834、124、968。\n- 題目在樹上求最大獨立集、直徑、距離和、覆蓋或每個根的答案。\n- 子問題自然落在子樹上。",
        how: "```cpp\narray<long long, 2> dfs(int u, int parent) {\n  long long skip = 0, take = value[u];\n  for (int v : graph[u]) {\n    if (v == parent) continue;\n    auto child = dfs(v, u);\n    skip += max(child[0], child[1]);\n    take += child[0];\n  }\n  return {skip, take};\n}\n```",
        mistakes:
          "- 無向樹 DFS 忘記 parent，走回父節點。\n- 父子限制未拆狀態，允許非法同選。\n- 合併多個孩子時覆蓋中間結果。\n- 換根時沒有扣掉孩子原本對父節點的貢獻。",
        checklist: [
          "狀態是否以子樹為單位？",
          "父子之間有哪些互斥或依賴？",
          "合併孩子時是加總、取 max，還是背包合併？",
          "是否需要每個節點作根的答案？",
        ],
      },
    ),
    sub(
      "DP 優化",
      "當轉移要查一批歷史最佳值，用前綴和、單調結構或資料結構把掃描降維。",
      {
        what: "DP 優化處理的是正確但太慢的轉移，例如 `dp[i] = max(dp[j] + cost(j,i))` 需要枚舉所有 j。常見工具有前綴和、單調佇列、Fenwick tree、線段樹、有序 map、二分、斜率優化與矩陣快速冪。",
        intuition:
          "先不要急著套資料結構，要先看轉移需要查什麼：前綴最大、固定窗口最大、某值域範圍最大、上一個相容位置，還是線性函數最值。把查詢型態說清楚，才知道要用哪個結構維護候選。",
        when: "- 1696、2945、3077、3186、2830、139 的 trie 優化、188 的 WQS 變形。\n- O(n²) DP 卡在枚舉轉移來源。\n- 候選有單調性、窗口限制、值域條件或可排序相容條件。",
        how: "```cpp\n// 單調佇列優化窗口最大轉移：dp[i] = nums[i] + max(dp[j])\ndeque<int> dq;\nfor (int i = 0; i < n; ++i) {\n  while (!dq.empty() && dq.front() < i - k) dq.pop_front();\n  dp[i] = nums[i] + (dq.empty() ? 0 : dp[dq.front()]);\n  while (!dq.empty() && dp[dq.back()] <= dp[i]) dq.pop_back();\n  dq.push_back(i);\n}\n```",
        mistakes:
          "- 還沒寫出正確 O(n²) 轉移就直接優化。\n- 查詢條件與插入順序不同步，把目前狀態提早加入。\n- 單調佇列未移除過期下標。\n- Fenwick/線段樹座標壓縮漏掉查詢邊界。",
        checklist: [
          "慢在哪個轉移枚舉？",
          "我要查的是前綴、窗口、值域還是相容位置？",
          "候選加入與查詢的時間順序正確嗎？",
